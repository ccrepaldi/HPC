...
!
      ...
      LQUERY = ( LWORK == -1 )
!
      if ( ( IJOB == 1 .OR. IJOB.EQ.2 ) .AND. NOTRAN ) then
         LWMIN = MAX( 1, 2*M*N )
      ELSE
         LWMIN = 1
      end if
!
      if ( .NOT.NOTRAN .AND. .NOT.LSAME( TRANS, 'T' ) ) then
         INFO = -1
      else if ( ( IJOB < 0 ) .OR. ( IJOB.GT.4 ) ) then
         INFO = -2
      else if ( M.LE.0 ) then
         INFO = -3
      else if ( N.LE.0 ) then
...
!
            if ( ISOLVE == 2 .AND. IROUND.EQ.1 ) then
               IFUNC = IJOB
               SCALE2 = SCALE
               ...
               DO 20 J = 1, N
                  ...
                  CALL DCOPY( M, ZERO, 0, F( 1, J ), 1 )
   20          CONTINUE
            else if ( ISOLVE == 2 .AND. IROUND.EQ.2 ) then
