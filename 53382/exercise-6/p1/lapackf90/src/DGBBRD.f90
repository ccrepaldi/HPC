...
!
      ...
      if ( .NOT.WANTQ .AND. .NOT.WANTPT .AND. .NOT.LSAME( VECT, 'N' ) ) &
         ...
         INFO = -8
      else if ( LDQ < 1 .OR. WANTQ .AND. LDQ.LT.MAX( 1, M ) ) then
         INFO = -12
      else if ( LDPT < 1 .OR. WANTPT .AND. LDPT.LT.MAX( 1, N ) ) then
         INFO = -14
      else if ( LDC < 1 .OR. WANTC .AND. LDC.LT.MAX( 1, M ) ) then
         INFO = -16
      end if
      if ( INFO.NE.0 ) then
