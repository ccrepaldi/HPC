...
!
      if ( .NOT.( WNTQA .OR. WNTQS .OR. WNTQO .OR. WNTQN ) ) then
         ...
         INFO = -5
      else if ( LDU < 1 .OR. ( WNTQAS .AND. LDU.LT.M ) .OR. &
               ( WNTQO .AND. M < N .AND. LDU.LT.M ) ) then
         INFO = -8
      else if ( LDVT < 1 .OR. ( WNTQA .AND. LDVT.LT.N ) .OR. &
               ( WNTQS .AND. LDVT < MINMN ) .OR. &
               ( WNTQO .AND. M.GE.N .AND. LDVT < N ) ) then
         INFO = -10
      end if
!
!     Compute workspace
...
!
      if ( INFO == 0 .AND. M.GT.0 .AND. N.GT.0 ) then
         if ( M.GE.N ) then
!
