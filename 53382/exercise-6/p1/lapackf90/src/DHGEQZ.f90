...
!
      ...
      if ( ISCHUR == 0 ) then
         ...
         INFO = -5
      else if ( IHI.GT.N .OR. IHI < ILO-1 ) then
         INFO = -6
      else if ( LDA < N ) then
         INFO = -8
      else if ( LDB < N ) then
         INFO = -10
      else if ( LDQ < 1 .OR. ( ILQ .AND. LDQ.LT.N ) ) then
         INFO = -15
      else if ( LDZ < 1 .OR. ( ILZ .AND. LDZ.LT.N ) ) then
         INFO = -17
...
!
               ...
               if ( .NOT.ILAZRO ) then
                  ...
                  if ( TEMPR < ONE .AND. TEMPR.NE.ZERO ) then
                     TEMP = TEMP / TEMPR
                     TEMP2 = TEMP2 / TEMPR
                  end if
...
!           (Single shift only.)
!
            if ( ( DBLE( MAXIT )*SAFMIN )*ABS( A( ILAST-1, ILAST ) ) <  &
